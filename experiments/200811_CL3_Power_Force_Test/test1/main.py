...
time.sleep(0.1)
param_velocity = float(2000)

print("Enter Power Setting (%)")
param_power_percent = float(sys.stdin.readline(100).rstrip())

# Log Session Settings
LOG_FOLDER_NAME = "data"
LOG_SESSION_FILENAME = "result_v" + str(int(param_velocity)) + "_p" + str(int(param_power_percent)) + ".csv"
...
with open(FILE_MULTIPLE_RESULT, "a") as rf: # Append
    ...
    rf.write(',')
    rf.write(str(param_velocity))
    rf.write(',')
    rf.write(str(param_power_percent))
    rf.write(',')
