...
SENSOR_CAL_SLOPE = 1.50e6  # 300kg LoadCell
SENSOR_CAL_INTERCEPT = -2.0e1 # 300kg LoadCell
...
# File Name formatting: result_manu_01.csv or result_batt_01.csv
found = False
LOG_SESSION_FILENAME = ""
FILE_LOG_SESSION = ""
for i in range (100):
    LOG_SESSION_FILENAME = "result_batt_%03i.csv" % i
    ...
    if os.path.isfile(FILE_LOG_SESSION):
        ...
        break
if found:
    print ("The log file will be saved to: %s. Press Enter to continue" % FILE_LOG_SESSION)
    input()
else:
...
with open(FILE_MULTIPLE_RESULT, "a") as rf: # Append
    ...
    rf.write(',')
    rf.write(str(LOG_SESSION_FILENAME))
    rf.write(',')
    rf.write(str(ch.maxVoltageRatio))
    rf.write(',')
    rf.write(str(ch.minForce))
    rf.write(',')
